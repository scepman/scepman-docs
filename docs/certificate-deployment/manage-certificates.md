...
{% hint style="info" %}
Only available when **Microsoft Intune** and/or **Jamf Pro** are used as MDM solution(s) for certificate enrollment. Alternatively it is available with any 3rd party MDM that is able to sync device and/or user objects with Azure AD (i.e. [Static AAD Validation](../advanced-configuration/application-settings/staticaad-validation.md) can be used).
{% endhint %}
...
{% hint style="success" %}
Supported on **OCSP** and [**CRL**](../scepman-configuration/optional/application-settings/crl.md).
{% endhint %}
